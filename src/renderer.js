...
class Renderer {
  render(templatePath, outputPath, data) {
    const { players, lastUpdated, historyData } = data;
  ...
  renderPlayer(p) {
    const { recent, teammates } = p.stats;
...
<tr class="player-row glass-card relative group cursor-pointer transition-transform duration-300 hover:scale-[1.01]" 
  ...
  <!-- Last Match Logic: Handle "Unknown" date if inconsistent -->
  <td class="p-4 text-xs text-white/40 font-mono text-right">${p.lastMatch}</td>
</tr>`.trim();
...
<div class="mb-4">
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 bg-[#0a0a14] border border-white/5 p-4 rounded-xl shadow-inner">
    ...
    <div><span class="text-white/30 block text-[10px] uppercase font-bold tracking-wider mb-1">Avg Kills</span> <span class="font-mono text-xl font-bold text-white">${Math.round(recent.kills / recent.matches)}</span></div>
...
<tr class="details-row hidden" data-player-id="${p.playerId}">
  <td colspan="7" class="p-0 border-none"> <!-- No padding, handled by inner div -->
